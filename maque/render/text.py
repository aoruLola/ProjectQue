...
def render_state(state: GameState, human_seat: str, last_event: ActionEvent | None) -> str:
    ...
    if last_event:
        tile_part = f" {tile_text_cn(last_event.tile)}" if last_event.tile else ""
    ...
    hand_str = " ".join(f"[{idx}]{tile_text_cn(tile)}" for idx, tile in idx_hand)
    lines.append(f"Hand({human_seat}): {hand_str}")

    lines.append("Discards:")
    for seat in ("E", "S", "W", "N"):
        recent = " ".join(tile_text_cn(t) for t in state.discard_view.recent_by_player[seat])
        lines.append(f"{seat}: [{recent}]")
    lines.append(f"History: {' '.join(tile_text_cn(t) for t in state.discard_view.history_compact)}")
...
def render_settlement(settlement) -> str:
    ...
    lines.append(f"Ma tiles: {' '.join(tile_text_cn(t) for t in settlement.ma_tiles) if settlement.ma_tiles else '(none)'}")
    lines.append("Delta:")
